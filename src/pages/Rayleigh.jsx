...
export default function Rayleigh() {
  ...
  const L = 2.0; // fixed screen distance for display
  const sep = deltaTheta * L; // physical separation on screen

  const rMax = 6 * (1.22 * lambda_m * L) / D_m;
  const nPts = 2000;
  ...
  // Dip depth: find the minimum between the two peaks
  ...
  // 2D: sum of two Airy disks
  const intensityFn2D = useCallback(
    (r) => {
      // This is a radial slice; for 2D we need x,y but approximate with offset along x
      return 0; // placeholder — real 2D is rendered via custom canvas below
    },
    []
  );
      ...
      <div className="flex-1 flex flex-col gap-4">
        <div className="bg-usna-card border border-usna-grid rounded-lg p-4" style={{ height: 420 }}>
          <IntensityPlot
            traces={traces}
            layoutOverrides={{
              xaxis: { title: { text: 'Screen Position (mm)' } },
            }}
          />
        </div>

        <RayleighDisk2D D_m={D_m} lambda_m={lambda_m} L={L} sep={sep} lambda={lambda} rMax={rMax} />
...
/**
 * Custom 2D canvas for two overlapping Airy disks.
 */
import { useRef, useEffect } from 'react';
import { setupCanvas } from '../lib/canvas';
import { wavelengthToRGB } from '../lib/color';

function RayleighDisk2D({ D_m, lambda_m, L, sep, lambda, rMax }) {
  ...
  useEffect(() => {
    ...
    for (let py = 0; py < pxSize; py++) {
      for (let px = 0; px < pxSize; px++) {
        ...
        const r1 = Math.sqrt((xPhys - sep / 2) ** 2 + yPhys ** 2);
        const r2 = Math.sqrt((xPhys + sep / 2) ** 2 + yPhys ** 2);
        const i1 = airyIntensity(r1, D_m, lambda_m, L);
        const i2 = airyIntensity(r2, D_m, lambda_m, L);
        const I = Math.min(1, (i1 + i2) / 2);
  ...
  }, [D_m, lambda_m, L, sep, lambda, rMax]);
